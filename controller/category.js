...
const addCategory=async(req,res,next)=>{
    try {
        ...
        res.status(200).json({code:200,status:true,message:"category added successfullyt"});
    } catch (error) {
        next(error);
    }
}
...
const getCategories=async(req,res,next)=>{
    try {
        ...
        res.status(200).json({code:200,status:true,message:"Get category list successful ",data:{categories,total,pages}})
    } catch (error) {
        next(error);
    }
}
